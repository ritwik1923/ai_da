...
from backend.app.agents.utility.custom_llm import CompanyGenAILLM, OllamaLocalLLM
...
"""
        ...
        def analyze_column(column_name: str) -> str:
            ...
            # Allow the LLM to pass either a raw column name or a code-like expression
            # e.g. "df['Customer Id'].unique()" or "df[\"Customer Id\"]"
            if isinstance(column_name, str):
                # try to extract column name from common patterns
                m = None
                try:
                    import re
                    m = re.search(r"df\[\s*['\"]([^'\"]+)['\"]\s*\]", column_name)
                except Exception:
                    m = None
                ...
                if m:
                    ...
                    # strip common method calls like .unique(), .tolist(), etc.
                    col = column_name.split(".")[0].strip().strip("'\"")
            else:
                col = column_name
    ...
    def _create_agent(self) -> AgentExecutor:
        ...
        # Different prompt templates for local vs cloud LLMs
        provider = settings.LLM_PROVIDER.lower()
...
Thought:{agent_scratchpad}"""
        ...
        # Reduce iterations and disable intermediate steps for local LLMs to speed up responses
        if provider == "local_llm":
            agent_max_iterations = settings.LOCAL_LLM_MAX_ITERATIONS

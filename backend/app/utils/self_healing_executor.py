...
class ReActExecutor:
    """
    ReAct (Reason + Act) pattern executor.
    Combines reasoning about data with code execution.
    """
    
    def __init__(self, df: pd.DataFrame, llm_callback: Callable):
        """
        Args:
            df: DataFrame to analyze
            llm_callback: Function(prompt) -> response for LLM reasoning
        """
        self.df = df
        self.llm_callback = llm_callback
        self.healing_executor = SelfHealingExecutor(df)
        self.thought_chain: List[Dict[str, str]] = []
    
    def execute_with_reasoning(self, query: str, max_steps: int = 5) -> Dict[str, Any]:
        """
        Execute query using ReAct pattern: Reason -> Act -> Observe -> Repeat.
        ...
        for step in range(1, max_steps + 1):
            ...
            self.thought_chain.append({"step": step, "type": "thought", "content": thought})
            
            # Act: Generate and execute code
            code = self._generate_code(thought)
            self.thought_chain.append({"step": step, "type": "action", "content": code})
            ...
            self.thought_chain.append({"step": step, "type": "observation", "content": observation})
            
            # Check if done
            if result.success and self._is_complete(observation, query):
                return {
                    "success": True,
                    "answer": self._format_answer(result.result),
                    "reasoning_chain": self.thought_chain,
                    "total_steps": step
                }
        ...
        return {
            "success": False,
            "error": "Max reasoning steps reached without complete answer",
            "reasoning_chain": self.thought_chain,
    ...
    def _reason(self, query: str, step: int) -> str:
        ...
        prompt = f"""
Step {step} of analyzing: "{query}"

Previous steps:
{self._format_thought_chain()}

What should be the next action to answer this query?
Respond with a specific action plan.
"""
        return self.llm_callback(prompt)
    
    def _generate_code(self, thought: str) -> str:
        """Generate code based on reasoning."""
        # This would use LLM to convert thought to code
        # For now, return a placeholder
        return "result = df.describe()"
    
    def _observe(self, result: ExecutionResult) -> str:
        """Analyze execution result."""
        if result.success:
            return f"Success! Got result: {result.result}"
        else:
            return f"Error: {result.error}"
    
    def _is_complete(self, observation: str, query: str) -> bool:
        """Check if the query is fully answered."""
        # Simple heuristic - check if we got a result
        return "Success" in observation
    ...
    def _format_thought_chain(self) -> str:
        """Format thought chain for prompt."""
        formatted = []
        for item in self.thought_chain[-3:]:  # Last 3 steps only
